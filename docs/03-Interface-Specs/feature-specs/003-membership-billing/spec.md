...
| Actor | Vai trò |
|---|---|
| Admin | CRUD gói; xem/giám sát membership, payment và báo cáo; huỷ đơn bất kỳ |
| Staff | Bán/gia hạn trực tiếp; ghi payment thủ công; huỷ đơn bất kỳ |
...
- **FR-MS-01 (Event):** WHEN Staff bán gói hợp lệ cho Member (chưa có gói Active còn hạn), THE system SHALL tạo Membership `PendingPayment` với `EndDate = StartDate + DurationDays`. WHERE member đã có đơn `PendingPayment`, THE system SHALL **tái dùng đơn đó** thay vì tạo trùng.
- **FR-MS-02 (Event):** WHEN Staff ghi nhận payment `Paid` cho membership `PendingPayment` (Amount ≥ giá gói), THE system SHALL chuyển Membership → `Active`, tái dùng bản `Payment` Pending nếu có, huỷ các đơn Pending anh em, ghi AuditLog `CONFIRM_PAYMENT`.
- **FR-MS-03 (Event):** WHEN Staff gia hạn trực tiếp (`/renew`, request `{packageId, method}`), THE system SHALL **kéo dài tại chỗ**: `EndDate = (EndDate hiện tại nếu còn hạn, ngược lại hôm nay) + DurationDays`, đặt `Active`, ghi nhận Payment `Paid` ngay. `method` nhận chuỗi `"cash"/"transfer"/"card"` (không phân biệt hoa thường).
...
- **FR-MS-04 (Optional):** WHERE người gọi là Member, THE system SHALL chỉ cho tạo *yêu cầu gia hạn* (`/renewal-request` → tạo đơn `PendingPayment`), Staff hoặc VNPay xác nhận thanh toán mới Active.
...
| POST | /api/v1/memberships/sell | Staff | {memberId, packageId, startDate?} | 201 {membership: PendingPayment} | 400, 403, 404, 409, 422 |
| POST | /api/v1/memberships/{id}/payment | Staff | {amount, method} | 201 {membership, payment, status} | 403, 404, 409, 422 |
| POST | /api/v1/memberships/{id}/renew | Staff | {packageId, method} | 200 MembershipResponse | 403, 404, 422 |
...
- IF Admin gọi API bán gói, gia hạn trực tiếp hoặc xác nhận payment thủ công, THEN 403 `FORBIDDEN`.

## 8. Acceptance Criteria (Given-When-Then)
- [ ] **AC-01:** Given gói 30 ngày, When Staff bán với StartDate=01/06, Then Membership PendingPayment, EndDate=01/07.
- [ ] **AC-02:** Given Membership PendingPayment, When Staff ghi payment Paid (≥ giá gói), Then status=Active + AuditLog `CONFIRM_PAYMENT`.
- [ ] **AC-03:** Given Member đang Active đến 01/07, When Staff gia hạn gói 30 ngày, Then EndDate mới=31/07 (nối tiếp).
...
- [ ] **AC-11:** Given người gọi có role Admin, When gọi `/sell`, `/{id}/payment` hoặc `/{id}/renew`, Then 403; Staff vẫn thực hiện được.
