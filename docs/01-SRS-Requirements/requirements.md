...
> **Nguồn chuẩn (canonical):** các FR chi tiết + mã lỗi + API nằm ở `docs/03-Interface-Specs/feature-specs/001-010/spec.md` (đã đồng bộ ngược từ code). Tài liệu này là bản tóm tắt yêu cầu ở mức cao. Vài điểm cần lưu ý: brute-force ghi vào `audit_logs` (không có bảng SecurityEvents riêng); "hôm nay" tính theo giờ VN (GMT+7).
...
### A9. Bổ sung đã implement (ngoài MVP gốc — chi tiết ở docs/03-Interface-Specs/feature-specs/)
