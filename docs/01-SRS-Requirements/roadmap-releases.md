...
| Tích hợp hạ tầng deploy trễ | TB | Demo localhost backup; deploy sớm. **Thực tế đã deploy Google Cloud Run (FE `gymmaster-os` + BE `gymmaster-api`) + Cloud SQL** (đổi từ Azure/Vercel) |
