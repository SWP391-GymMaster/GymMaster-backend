...
| D-27 | 2026-07-24 | **Khôi phục MemberProfile đã soft-delete tại chỗ** trong flow relink và `/members/me`, không chèn profile thứ hai | `member_profiles.UserId` có unique index không filter. FR-MEM-04 giữ row khi xóa mềm, còn FR-MEM-06 tự tạo khi không thấy profile active; nếu luôn INSERT, SQL Server trả unique violation và làm mất liên kết lịch sử nếu đổi ID. Restore cùng row là cách duy nhất thỏa cả hai rule mà không đổi schema/API | Giữ nguyên `MemberId`, dữ liệu profile và các quan hệ membership/payment/progress/assignment; set `IsDeleted=0`, cập nhật `UpdatedAt`, ghi `RESTORE_MEMBER`. Muốn đổi dữ liệu sau restore phải dùng `PUT`. Profile chỉ được restore khi User role Member còn tồn tại và chưa soft-delete |
| D-28 | 2026-07-26 | **Tách quyền giao dịch Membership/Billing: chỉ Staff bán gói, gia hạn trực tiếp và xác nhận payment thủ công**; Member chỉ trả đơn của mình qua VNPay, Staff có thể khởi tạo thay; Admin chỉ quản trị gói và xem/giám sát dữ liệu | Admin là chủ phòng/quản trị, Staff là người vận hành quầy. Cho Admin cùng thao tác giao dịch làm trùng trách nhiệm, trái luồng FE Staff-only và giảm khả năng truy trách nhiệm | `POST /memberships/sell`, `/{id}/payment`, `/{id}/renew` dùng `[Authorize(Roles="staff")]`; `VnPayService.CanInitiate` chỉ nhận Staff hoặc Member sở hữu; Admin gọi các thao tác này nhận 403. Quyền đọc billing và quyền huỷ đặc biệt của Admin giữ nguyên |
