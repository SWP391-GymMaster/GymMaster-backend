...
public sealed class FoodScanService : IFoodScanService
{
    ...
    public async Task<AuthServiceResult<FoodScanResponse>> ScanImageAsync(
        IFormFile? image,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return AuthServiceResult<FoodScanResponse>.Failure(
        ...
        return AuthServiceResult<FoodScanResponse>.Success(new FoodScanResponse(items));
    }

    public async Task<AuthServiceResult<ScannedFood>> ConfirmAiFoodAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (actorId is null || !principal.IsInRole(RoleNames.Member) ||
            !await HasActivePackageAsync(actorId.Value, cancellationToken))
        {
            return AuthServiceResult<ScannedFood>.Failure(
                "MEMBERSHIP_REQUIRED", UpgradeMessage, StatusCodes.Status403Forbidden);
        }
        ...
        {
            return AuthServiceResult<ScannedFood>.Success(ToScannedFood(existing));
        }
        ...
        return AuthServiceResult<ScannedFood>.Success(ToScannedFood(foodItem), StatusCodes.Status201Created);
    }
