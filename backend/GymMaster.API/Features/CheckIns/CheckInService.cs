...
public sealed class CheckInService : ICheckInService
{
    ...
    public async Task<ServiceResult<CheckInResponse>> CreateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return ServiceResult<CheckInResponse>.Success(ToResponse(checkIn), StatusCodes.Status201Created);
    }

    // GET /api/v1/checkins?date=&memberId=  (Admin, Staff)
    public async Task<ServiceResult<IReadOnlyList<CheckInResponse>>> ListAsync(
        DateOnly? date,
        long? memberId,
        CancellationToken cancellationToken)
    {
        ...
        return ServiceResult<IReadOnlyList<CheckInResponse>>.Success(
            rows.Select(ToResponse).ToList());
    }

    // GET /api/v1/members/{id}/checkins  (Admin, Staff, PT, Member self)
    public async Task<ServiceResult<IReadOnlyList<CheckInResponse>>> ListByMemberAsync(
        long memberId,
        ClaimsPrincipal principal,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<ServiceResult<CheckInResponse>> CreateForAssignedMemberAsync(
        long memberId,
        ClaimsPrincipal principal,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<ServiceResult<IReadOnlyList<CheckInResponse>>> ListTodayForTrainerAsync(
        ClaimsPrincipal principal,
        CancellationToken cancellationToken)
    {
        var actorUserId = GetActorId(principal);
    ...
    private async Task<ServiceResult<CheckInResponse>?> ValidateMembershipAsync(
        long memberId,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<ServiceResult<CheckInResponse>?> ValidateDailyLimitAsync(
        long memberId,
        CancellationToken cancellationToken)
    {
    ...
    private static ServiceResult<IReadOnlyList<CheckInResponse>> Ok(IEnumerable<CheckIn> items)
    {
        return ServiceResult<IReadOnlyList<CheckInResponse>>.Success(
    ...
    private static ServiceResult<CheckInResponse> Fail(string code, string message, int statusCode)
    {
        return ServiceResult<CheckInResponse>.Failure(code, message, statusCode);
    }
