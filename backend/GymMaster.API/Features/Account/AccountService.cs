...
public sealed class AccountService : IAccountService
{
    ...
    public async Task<ServiceResult<AuthUserResponse>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return ServiceResult<AuthUserResponse>.Success(
            ToUserResponse(user, await GetMemberProfileIdAsync(user.Id, cancellationToken)));
    }

    public async Task<ServiceResult<AuthUserResponse>> UploadAvatarAsync(
        ClaimsPrincipal principal,
        IFormFile? file,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<ServiceResult<PersonalProfileResponse>> GetProfileAsync(
        ClaimsPrincipal principal,
        CancellationToken cancellationToken)
    {
        var user = await GetUserFromPrincipalAsync(principal, cancellationToken);
        ...
        {
            RoleNames.Admin or RoleNames.Staff => ServiceResult<PersonalProfileResponse>.Success(
    ...
    public async Task<ServiceResult<PersonalProfileResponse>> UpdateProfileAsync(
        ClaimsPrincipal principal,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (role is RoleNames.Admin or RoleNames.Staff)
        {
            ...
            return ServiceResult<PersonalProfileResponse>.Success(ToPersonalProfileResponse(profile));
        }
    ...
    private async Task<ServiceResult<PersonalProfileResponse>> GetTrainerPersonalProfileAsync(
        long userId,
        CancellationToken cancellationToken)
    {
        var profile = await _dbContext.TrainerProfiles
        ...
        return profile is null
            ? Fail<PersonalProfileResponse>(
                ...
                "Chua co ho so huan luyen vien - lien he quan tri vien.",
                StatusCodes.Status404NotFound)
            : ServiceResult<PersonalProfileResponse>.Success(ToPersonalProfileResponse(profile));
    }
