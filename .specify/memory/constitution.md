...
ARCH-01: Vertical slice `Features/<Tên>/` (Controller + Service + DTO); Controller mỏng, không truy vấn DB; Service gọi thẳng `DbContext`, KHÔNG có tầng Repository; business rule dùng chung gom về một nguồn. ARCH-02: response chuẩn `{success, data, error, meta}`; lỗi `{code, message, requestId}` không lộ stack trace. ARCH-03: RBAC 4 role (Admin/Staff/PT/Member) + ownership check ở Service. ARCH-04: Database First — schema do team DB sở hữu qua script SQL trong `database/`, KHÔNG dùng EF Migration; backend chỉ tiêu thụ schema.
...
Coverage ≥ 80% business logic; mỗi endpoint có happy path + ≥1 error case. Trước merge phải qua Validation Gate 4 lớp (Automated → Spec compliance → Constitution → Acceptance) — xem `docs/04-Test-Specs/test-plan.md` §6. Không tin "AI đã xong"; tin khi test xanh + checklist tick.
...
## Governance
Hiến pháp này vượt trên mọi thực hành khác. Mọi PR/review phải xác minh tuân thủ Core Principles. Vi phạm Hard Rule (SEC-*/DATA-*/AUDIT-*) chặn merge và phải escalate. Sửa đổi hiến pháp: ghi vào `docs/06-Management/decision-log.md`, tăng version, đồng bộ cả `CONSTITUTION.md` và file này. Hướng dẫn runtime cho agent: `docs/06-Management/agents.md` + `CLAUDE.md`.

**Version**: 1.3.0 | **Ratified**: 2026-05-30 | **Last Amended**: 2026-07-23
